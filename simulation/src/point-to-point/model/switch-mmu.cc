...
namespace ns3 {
	...
	bool SwitchMmu::CheckIngressAdmission(uint32_t port, uint32_t qIndex, uint32_t psize){
		if (psize + hdrm_bytes[port][qIndex] > headroom[port] * 2 && psize + GetSharedUsed(port, qIndex) > GetPfcThreshold(port) * 2){
			printf("buffer_size=%lu, reserve=%lu, resume_offset=%lu, shared_used_bytes=%lu, headroom[%u]=%lu, GetPfcThreshold(%u)=%lu\n", buffer_size, reserve, resume_offset, shared_used_bytes, port, headroom[port], port, GetPfcThreshold(port));
